...
export function AutoSaveStatusPill({ status }: { status: AutoSaveStatus }) {
  ...
  // timer keeps ticking after the parent flips back to "idle". Set-
  // state-during-render with a sentinel is required here — useEffect
  // → setState trips react-hooks/set-state-in-effect; the timestamp
  // comes from the status itself so the body stays pure.
  ...
  if (status.kind === "saving") {
    return (
      <Pill cls="bg-white text-ink border-light-blue">
        <Spinner /> Saving…
      </Pill>
    );
  }
  if (status.kind === "error") {
    return (
      <Pill cls="bg-red-50 text-red-800 border-red-300 dark:bg-red-900/30 dark:text-red-200 dark:border-red-700">
  ...
  return (
    <Pill cls="bg-green-50 text-green-800 border-green-300 dark:bg-green-900/30 dark:text-green-200 dark:border-green-700">
