 ...
 * rendering one PromptEditor per prompt row) — each editor dispatches
 * via `useAutoSaveDispatch()` and the single pill at the bottom-right
 * reflects whichever save fired most recently.
 *
 * Pages with a single editor can skip the provider and pass status
 * directly to AutoSaveStatusPill instead.
 */
...
/**
 * Returns a `setStatus` function for the enclosing AutoSaveProvider.
 * Throws if used outside one — pages that don't host a provider
 * should pass status to AutoSaveStatusPill directly.
 */
