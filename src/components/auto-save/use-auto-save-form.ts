 ...
 *
 * Caller wires `save` to whatever persistence path the form uses. The
 * hook only fires when `isFormDirty(form)` is true, so consecutive
 * triggers collapse to a single save.
 *
 * Set the form to `onSubmit={(e) => e.preventDefault()}` so Enter
 * inside an input doesn't try to submit through the (removed) action.
 */
export function useAutoSaveForm({
  ...
  save: () => void;
  debounceMs?: number;
  /** Resets the debounce timer when the row's updated_at changes. */
  freshnessKey: string;
}) {
...
/**
 * Build a stable callback that snapshots the form's current FormData
 * and runs it through whatever persistence path the caller provides.
 * Memoized so the auto-save hook can hold a stable reference.
 */
