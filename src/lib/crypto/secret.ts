...
// vars + .env.local. Rotation is a separate operation (M6.17 in the suite
// plan ships scripts/rotate-student-gdrive-key.sh).
...
export function readKeyFromEnv(): Buffer {
  const raw = process.env.STUDENT_GDRIVE_TOKEN_ENC_KEY;
  if (!raw) {
    throw new Error(
      "STUDENT_GDRIVE_TOKEN_ENC_KEY is not configured — cannot encrypt/decrypt student Google tokens. Generate with `openssl rand -base64 32` and add to env.",
    );
  }
  const key = Buffer.from(raw, "base64");
  if (key.length !== 32) {
    throw new Error(
      `STUDENT_GDRIVE_TOKEN_ENC_KEY must decode to exactly 32 bytes (got ${key.length}). Re-generate with \`openssl rand -base64 32\`.`,
    );
  }
  return key;
}
 ...
 * silently falling back to plaintext re-opens the at-rest leak Phase 0c
 * closes.
 */
